...
const DEFAULT_W_PIXELS: usize = 512;
const DEFAULT_H_PIXELS: usize = 512;

fn main() {
    let args: Vec<String> = env::args().collect();
    let mut w = DEFAULT_W_PIXELS;
    let mut h = DEFAULT_H_PIXELS;
    let mut s = DEFAULT_SAMPLES;

    match args.len() {
        2 => s = args[1].parse().unwrap(),
        4 => {
            s = args[1].parse().unwrap();
            w = args[2].parse().unwrap();
            h = args[3].parse().unwrap();
        },
        _ => {},
    }
